...
final class FuelStatisticsTextViewController: FuelStatisticsViewController {
  ...
  override func computeStatisticsForRecentMonths(_ numberOfMonths: Int, forCar car: Car, withObjects fetchedObjects: [FuelEvent], inManagedObjectContext moc: NSManagedObjectContext) -> DiscardableDataObject {
    ...
    var state: FuelStatisticsData! = contentCache[numberOfMonths] as? FuelStatisticsData
    ...
    if state.contentImage == nil {
      ...
      let renderer = UIGraphicsImageRenderer(size: CGSize(width: scrollView.bounds.size.width, height: height))
  ...
  override func viewDidLayoutSubviews() {
    ...
    gridLeftBorder = gridMargin
    gridRightBorder = view.bounds.size.width - gridMargin
    gridDesColumnWidth = (view.bounds.size.width - gridMargin - gridMargin) / 2.0
  ...
  private func drawBackground(_ context: CGContext) {
    ...
    context.fill(CGRect(x: 0, y: 0, width: view.bounds.size.width, height: 28))
  }

  private func drawStatisticsForState(_ state: FuelStatisticsData, withHeight height: CGFloat, context: CGContext) {
    UIColor.clear.setFill()
    context.fill(CGRect(x: 0, y: 0, width: view.bounds.size.width, height: height))
    ...
    if state.numberOfFillups == 0 {
      ...
      x = floor((view.bounds.size.width - size.width) / 2.0)
      y = floor((view.bounds.size.height - (size.height - font.descender)) / 2.0)
      ...
      path.move(to: CGPoint(x: gridLeftBorder, y: 1.0))
      path.addLine(to: CGPoint(x: gridRightBorder, y: 1.0))
      ...
      path.removeAllPoints()
      path.move(to: CGPoint(x: gridLeftBorder, y: 0.25))
      path.addLine(to: CGPoint(x: gridRightBorder, y: 0.25))
      ...
      path.removeAllPoints()
      path.move(to: CGPoint(x: gridLeftBorder + gridDesColumnWidth + 0.25, y: 0.0))
      path.addLine(to: CGPoint(x: gridLeftBorder + gridDesColumnWidth + 0.25, y: gridTextHeight * CGFloat(gridLines)))
      path.stroke()
      ...
      func drawEntry(label: String, value: String) {
        ...
        let x1 = gridLeftBorder + gridDesColumnWidth - size.width - gridTextXMargin
        label.draw(at: CGPoint(x: x1, y: y), withAttributes: labelAttributes)

        let x2 = gridLeftBorder + gridDesColumnWidth + gridTextXMargin
  ...
  override func displayCachedStatisticsForRecentMonths(_ numberOfMonths: Int) -> Bool {
    let cell = contentCache[numberOfMonths] as? FuelStatisticsData
    ...
    if let contentImage = cell?.contentImage {
      ...
      var imageView: UIImageView! = scrollView.viewWithTag(1) as? UIImageView
